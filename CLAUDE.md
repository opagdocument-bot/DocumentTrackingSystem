...
- `mobile/` — the liaison's Expo app (SDK 57), which imports the office rules from `app/src`
...
`mobile/` is the liaison's phone app — Expo SDK 57, pinned to what the office's Expo Go supports.
Only the liaison uses it; every other role works from the web.

Expo Go auto-updates from the Play/App Store and only ever supports the one SDK its current
release targets, so this number drifts out from under the project without warning. Before assuming
a "project is incompatible" error means the *project* is on the wrong SDK, read the error carefully —
it names both versions, and which one is "the installed version of Expo Go" vs "the project you
opened" is easy to misread backwards. Confirm the true state with `npx expo install --check` (must
say "up to date") and the running dev server's own manifest (`curl -H "expo-platform: android"
http://127.0.0.1:8081/` — the `runtimeVersion` field), rather than trusting either party's memory of
what the phone last supported. To upgrade: `npx expo install expo@<version>` then `npx expo install
--fix`, which reconciles react, react-native and every Expo package in one pass. Stop any running
`expo start` first — Metro holds file handles inside `node_modules` open, and `npm install` fails
with `EPERM: operation not permitted, rmdir` on Windows while it does.
...
`https://claude.ai/code/artifact/e2adf4c4-0211-4bb7-a809-141a9926aa47`). Always end the reply with
that link.
...
`makeDB(mode)` serves three seed sets: `empty` (no documents — **the default**, since a fresh install
of the real system starts with none), `walkthrough` (3 documents, one per acting role) and `full`
(33 documents). Switchable from the sidebar. The mobile app has no such switch — it always starts
`empty`; it has no backend to receive real documents from yet, so there is nothing else it could show.
